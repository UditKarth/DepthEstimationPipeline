import React, {
  ...
  class SimpleOrbitControls {
    ...
    pan(dx, dy) {
      ...
      const targetDistance =
        offset.length() * Math.tan(((this.camera.fov / 2) * Math.PI) / 180.0);
  ...
  function seededRandom(seed) {
    let s = Math.floor(Number(seed)) || 1;
    s = s % 2147483647;
    if (s <= 0) s += 2147483646;
    return function () {
  ...
  const ScenePerceptionApp = React.memo(function ScenePerceptionApp() {
    ...
    /** @param {number} [seedOverride] - if provided, use this seed for this run and update state (so Run button visibly changes the scene) */
    const regenerateScene = useCallback((seedOverride) => {
      const raw = seedOverride !== undefined ? seedOverride : seed;
      const s = Math.floor(Number(raw)) || 1;
      if (s !== Number(seed)) setSeed(s);
      const prng = seededRandom(s);
      const primitives = [];
      let totalVerts = 0;
      let totalTris = 0;
    ...
    /** @param {typeof virtualCameras} camerasOverride - if provided (e.g. from buildCameraRig()), use this instead of state so Run works without waiting for state update */
    const runRaycasting = useCallback((camerasOverride) => {
      const camsSource = camerasOverride ?? virtualCameras;
      if (!scenePrimitives.length || !camsSource.length) return;
      const cams = camsSource.map((c) => ({
        ...c,
    ...
    const runPointCloudFusion = useCallback(() => {
      ...
      // Statistical outlier removal: mean distance to k nearest neighbors.
      // Use a spatial grid so we only check nearby points (O(N) instead of O(N²)).
      const k = 8;
      const N = fused.count;
      const meanDistances = new Float32Array(N);
      const pos = fused.positions;
      ...
      const grid = new Map();
      for (let i = 0; i < N; i++) {
        const ix = Math.floor((pos[i * 3] - minX) / cellSize);
        const iy = Math.floor((pos[i * 3 + 1] - minY) / cellSize);
        const iz = Math.floor((pos[i * 3 + 2] - minZ) / cellSize);
        const key = `${ix},${iy},${iz}`;
      ...
      for (let i = 0; i < N; i++) {
          ...
          for (let dy = -1; dy <= 1; dy++) {
            for (let dz = -1; dz <= 1; dz++) {
              ...
              for (let u = 0; u < cell.length; u++) {
                const j = cell[u];
                if (j === i) continue;
                tmpB.set(pos[j * 3], pos[j * 3 + 1], pos[j * 3 + 2]);
        ...
        if (dists.length === 0) {
          meanDistances[i] = Infinity;
          continue;
        }
        dists.sort((a, b) => a - b);
        const useLen = Math.min(k, dists.length);
        let sum = 0;
        for (let t = 0; t < useLen; t++) sum += dists[t];
        meanDistances[i] = sum / useLen;
      }
      let mu = 0;
      let finiteCount = 0;
      for (let i = 0; i < N; i++) {
        const v = meanDistances[i];
        if (Number.isFinite(v)) { mu += v; finiteCount++; }
      }
      mu = finiteCount > 0 ? mu / finiteCount : 0;
      let sigma = 0;
      for (let i = 0; i < N; i++) {
        const v = meanDistances[i];
      ...
      sigma = finiteCount > 0 ? Math.sqrt(sigma / finiteCount) : 0;
    ...
    const runICP = useCallback(() => {
      ...
      for (let i = 0; i < targetPoints.length; i++) {
        ...
        if (t.x < tMinX) tMinX = t.x; if (t.x > tMaxX) tMaxX = t.x;
        if (t.y < tMinY) tMinY = t.y; if (t.y > tMaxY) tMaxY = t.y;
        if (t.z < tMinZ) tMinZ = t.z; if (t.z > tMaxZ) tMaxZ = t.z;
      }
      const targetGrid = new Map();
      for (let i = 0; i < targetPoints.length; i++) {
        const t = targetPoints[i];
        const gx = Math.floor((t.x - tMinX) / cellSizeIcp);
        const gy = Math.floor((t.y - tMinY) / cellSizeIcp);
        const gz = Math.floor((t.z - tMinZ) / cellSizeIcp);
        const key = `${gx},${gy},${gz}`;
        if (!targetGrid.has(key)) targetGrid.set(key, []);
        targetGrid.get(key).push(i);
      }

      const findClosest = (p) => {
        const gx = Math.floor((p.x - tMinX) / cellSizeIcp);
        const gy = Math.floor((p.y - tMinY) / cellSizeIcp);
        const gz = Math.floor((p.z - tMinZ) / cellSizeIcp);
        let bestIdx = 0;
        ...
        for (let dx = -1; dx <= 1; dx++) {
          for (let dy = -1; dy <= 1; dy++) {
            for (let dz = -1; dz <= 1; dz++) {
              ...
              const cell = targetGrid.get(key);
              if (!cell) continue;
              for (let u = 0; u < cell.length; u++) {
                const i = cell[u];
                const t = targetPoints[i];
                const dx_ = p.x - t.x, dy_ = p.y - t.y, dz_ = p.z - t.z;
                const dSq = dx_ * dx_ + dy_ * dy_ + dz_ * dz_;
                if (dSq < bestDistSq) {
                  bestDistSq = dSq;
                  bestIdx = i;
                }
  ...
  const runReconstruction = useCallback(() => {
    ...
    const cellSizePoint = Math.max(radius * 1.01, 1e-6);
    const pointGrid = new Map();
    for (let idx = 0; idx < points.length; idx++) {
      const pt = points[idx];
      const gx = Math.floor((pt.x - min.x) / cellSizePoint);
      const gy = Math.floor((pt.y - min.y) / cellSizePoint);
      const gz = Math.floor((pt.z - min.z) / cellSizePoint);
      const key = `${gx},${gy},${gz}`;
    ...
    for (let k = 0; k < nz; k++) {
      for (let j = 0; j < ny; j++) {
        for (let i = 0; i < nx; i++) {
          ...
          pVec.set(x, y, z);
          const gx = Math.floor((x - min.x) / cellSizePoint);
          const gy = Math.floor((y - min.y) / cellSizePoint);
          const gz = Math.floor((z - min.z) / cellSizePoint);
          let num = 0;
          let den = 0;
          for (let dz = -1; dz <= 1; dz++) {
            for (let dy = -1; dy <= 1; dy++) {
              for (let dx = -1; dx <= 1; dx++) {
                ...
                if (!cell) continue;
                for (let u = 0; u < cell.length; u++) {
                  const pt = points[cell[u]];
                  qVec.set(pt.x, pt.y, pt.z);
                  const d = pVec.distanceTo(qVec);
                  if (d > radius) continue;
                  nVec.set(pt.nx, pt.ny, pt.nz);
                  const w = (pt.conf || 1) / (d + 1e-3);
                  const sdfVal = pVec.clone().sub(qVec).dot(nVec);
                  num += w * sdfVal;
                  den += w;
                }
              }
    ...
    // Use Three.js MarchingCubes addon (see https://threejs.org/docs/#MarchingCubes)
    const material = new THREE.MeshStandardMaterial({
      color: 0x88aacc,
      metalness: 0.1,
      roughness: 0.6,
    });
    const maxPolyCount = 500000;
    const mc = new MarchingCubes(nx, material, false, false, maxPolyCount);
    mc.isolation = 0; // our SDF: inside when value < 0
    for (let k = 0; k < nz; k++) {
      for (let j = 0; j < ny; j++) {
        for (let i = 0; i < nx; i++) {
          mc.setCell(i, j, k, sdf[idx3D(i, j, k)]);
        }
      }
    }
    mc.update();

    // MarchingCubes output is in [0, 2]^3; scale and position to world bounds
    const spanX = max.x - min.x;
    const spanY = max.y - min.y;
    const spanZ = max.z - min.z;
    mc.scale.set(spanX / 2, spanY / 2, spanZ / 2);
    mc.position.set(min.x + spanX / 2, min.y + spanY / 2, min.z + spanZ / 2);

    const geom = mc.geometry;
    const vertexCount = geom.attributes.position?.count ?? 0;
    const faceCount = geom.index ? geom.index.count / 3 : 0;
  ...
  const handleRunStage = (s) => {
    if (s === 1) {
      const nextSeed = (Math.floor(Number(seed)) || 1) + 1;
      regenerateScene(nextSeed);
      setMaxUnlockedStage((m) => Math.max(m, 1));
    } else if (s === 2) {
      const cams = buildCameraRig();
      if (cams?.length && scenePrimitives.length) runRaycasting(cams);
